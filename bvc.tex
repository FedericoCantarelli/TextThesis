...
\end{figure}
Even if the height observations are taken nominally at the same age for each girl, it is not strictly necessary for this to happen. Indeed, the points at which the function is observed may change from one subject to another. This is especially important in quality control that relies on sensors we discussed in Section \ref{sec:sensoriniiniini}. The signals transmitted by these sensors may represent disturbances that prevent the observed quantity from being correctly measured, causing discontinuities or missing values. It is also important to notice that FDA does not suffer from the problems of classical multivariate methods. In fact, it can also be used when the number of variables $p$ is larger than the available sample size $n$. In case of Fig. \ref{fig:tuttoinsieme}, we have $p=31$ and $n=10$. In general, the goal of the FDA is to represent data as functions, with the aim of providing a representation that helps analysis and highlights the various characteristics of the collected data. This allows us to explain patterns of variation in the data and to compare data containing different sets of observations of the same function or different functions for a common set of replicates. We can write each functional observation as the sum of a functional component and a random component that we will call noise. Formally, we can write every profile $y_j$ as 
\begin{equation}
...
\end{equation}
When $K=n$ we have a special situation called exact representation or interpolation. It is a limit situation because we can choose the coefficients of the weighted sum such that $\hat{f}(t_j)=y_j$ for each $j$. Thus, $K$ controls the smoothness of the approximation $\hat{f}(t)$. We have to choose this parameter according to used functional basis and the characteristics of the data. Another parameter to be chosen is the type of functional basis. Indeed, there are different functional bases, each with different characteristics, which should ideally reflect the features of observed data. In a certain way, we can use the parameter $K$ to understand whether the used basis was more or less suitable: the smaller $K$ is, the better the base reflects the characteristics of the data. The type of basis selected is particularly important when one is interested in analysing derivatives. Formally, when we are interested in
\begin{equation}
    \label{eq:derivative}
...
\end{equation}
where $\omega$ is a shift parameters that is usually chosen to be in the center of the interval of approximation. The polynomial basis tends to fit the centre of the data very well, but the tails tend to have less ideal behaviour. This is why they are never used for extrapolation or forecasting. However, the basis exhibits a desirable behaviour: $t_j$ are equally spaced, the basis can be fit with very little computational effort. Moreover, derivatives are simple to compute and they tend to be good estimators for the true derivatives.
...
\paragraph{Fourier basis.} One of the most famous and most widely used bases is the Fourier basis, mainly because it is able to capture the periodic behaviour of data, but also because it is very usefull for stable functions and when curvatures tent to be of the same order in all the data. However, Fourier basis is not suitable to data known or suspected to reflect some kind of discontinuities. Fourier basis is defined as:
\begin{equation}
    \label{eq:fourier}
...
\paragraph{B-splines basis.} The B-splines base is a splines base. Splines are basis defined by a number $L$ of knots and an order $m$. Knots are values $\tau_l$ for $l=1,\dots,L-1$ that divide the interval on which a function has to be estimated into $L$ sub-intervals. Remember that every sub-interval must contain at least one observation. On each sub-interval, the spline is a polynomial function of a specific order $m$, which is the degree of the polynomial itself. Polynomials defined on adjacent intervals join smoothly at knots value, such that the function at that point is constrained to the value of the knot itself. By increasing the numbers of knots, the fit of the estimated function to the observed data can be improved, up to the extreme situation where the knot numbers have the same value as the observed data. The number of nodes will therefore affect the goodness of fit to the observed data. In a certain way, the parameter $L$ regulates the flexibility of our fit. This is why it is a good practice for regions characterised by complex variations to have more nodes. Thus, to summarise what has been said so far, a spline is defined by two elements: the order of the polynomials and the sequence of nodes.
B-splines are a particular spline basis developed by de Boor in 2001. 
\begin{figure}
\centering
...
\end{equation}
for integers $j$ and $k$. The wavelet expansion of a function $f$ gives a multiresolution analysis in the sense that the coefficient of $\psi_{jk}$ yields information about $f$ near position $2^{-j}k$ on scale $2^{-j}$, i.e., at frequencies near $c2^j$ for some constant $c$. Wavelets are good for modeling sharp local features even discontinuity, localized in both space and frequency, which can be computed in an efficient way. Multidimensional wavelets provides coherent estimates of ECG traces, where the different projections of the heart dynamics are among them consistent.
\paragraph{Fit model to data.} There are different ways to fit a model to observed data. When we have to estimate a model, we want the model to fit our data in some way. To quantify how well it fits, we can define the concept of sum of squared errors defined as
\begin{equation}
    \label{eq:sse}
...
\end{equation}
\paragraph{Choose the number $K$ of basis functions.} As we have already seen in previous paragraphs, by increasing the number of basis function $K$ we can increase the goodness of fit. Indeed, the larger $K$, the better the fit to data. However, by increasing $K$ disproportionately, our model would also capture the stochastic component of the noise. On the other hand, by decreasing $K$ too much, we may risk missing some features of the data. it is the classic problem of the trade-off between bias and variance. The bias is defined as
\begin{equation}
    \label{eq:bias}
